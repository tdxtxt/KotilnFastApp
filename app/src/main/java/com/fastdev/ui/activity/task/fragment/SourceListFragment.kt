...
class SourceListFragment : BaseMvpFragment(), SourceListPresenter.BaseMvpImpl {
    ...
    private fun load(){
        when(type){
            SourceBean.STATUS_ALL ->{
                presenter.querySourceAll(viewModel?.taskId, pageNum)
            }
            SourceBean.STATUS_WAIT ->{
                presenter.querySourceByWait(viewModel?.taskId, pageNum)
            }
            SourceBean.STATUS_PY ->{
                presenter.querySourceByPY(viewModel?.taskId, pageNum)
            }
            SourceBean.STATUS_PK ->{
                presenter.querySourceByPK(viewModel?.taskId, pageNum)
            }
            SourceBean.STATUS_FINISH ->{
                presenter.querySourceByFinish(viewModel?.taskId, pageNum)
            }
        }
    }
