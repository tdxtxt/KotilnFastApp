   ...
   public AudioPlayManager play(String audioPath , Context context, AudioPlayMode mode){
       if (isPermission(context)) {
           if ((iAudioPlay).getStatus() == AudioPlayStatus.PLAYING && (iAudioPlay).getAudioPath().equals(audioPath)) {
               ...
               if(!playThread.isAlive()) executors.execute(playThread);
               if(playThread != null && playThread.handler != null){
     ...
     * 播放音频
     */
    public AudioPlayManager play(String audioPath, Context context, AudioPlayMode mode, AudioPlayListener playListener){
        if(iAudioPlay.getAudioPlayListener() != null) iAudioPlay.getAudioPlayListener().onStop();
        play(audioPath,context,mode);
        (iAudioPlay).setAudioPlayListener(playListener);
        return this;
    }
   ...
   public AudioPlayStatus getPlayStatus(){
    ...
    public boolean isPermission(Context context) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP
                ...
                == PermissionChecker.PERMISSION_DENIED && ActivityCompat.checkSelfPermission(context, Manifest.permission.WRITE_EXTERNAL_STORAGE)
